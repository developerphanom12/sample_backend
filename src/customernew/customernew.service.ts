...
export class CustomerService {
  constructor(
    ...
    private customeraccRepository: Repository<CustomerEntity>,
  ) {}
  ...
  private async extractCompanyFromActiveAccount(active_account: string) {
    ...
    const company = await this.companyRepository.findOne({
      ...
      relations: ['sales', 'currency', 'customerNEWaccount'],
    });
  ...
  async createSupplier(id: string, body: CustomerDto) {
    ...
    const customerAcc = await this.customeraccRepository.save({
      name: body.name,
      sale_invoices:body.sale_invoices,
  ...
  async updateSupplier(id: string, body: UpdateCustomerDTO) {
    const company = body.active_account
      ? await this.extractCompanyFromActiveAccount(body.active_account)
      : await this.extractCompanyFromUser(id);
    ...
    if (!customnerId) {
      throw new HttpException(
        'REQUEST SHOULD CONTAIN CUSTOMERS ID',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    await this.customeraccRepository.update(customer.id, {
      name: body.name,
      sale_invoice:body.sale_invoice
    });
